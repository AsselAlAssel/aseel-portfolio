...
export function HeroVisual() {
  ...
  const canParallax = !reducedMotion && typeof window !== 'undefined' && window.matchMedia('(pointer: fine)').matches;
  const { x, y } = useMousePosition(canParallax);
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{ x: x * 8, y: y * 6, rotateY: x * 1.8, rotateX: y * -1.2 }}
      >
        <img
          src={heroImage}
      ...
      <FloatingTechBadge label="React" mark="⚛" className="left-[2%] top-[46%]" x={x} y={y} depth={-16} delay={0.86} />
      <FloatingTechBadge label="TypeScript" mark="TS" className="right-[1%] top-[32%]" x={x} y={y} depth={16} delay={0.98} />
      <FloatingTechBadge label="Next.js" mark="N" className="right-[4%] bottom-[8%]" x={x} y={y} depth={14} delay={1.08} />
    </motion.div>
  );
}
